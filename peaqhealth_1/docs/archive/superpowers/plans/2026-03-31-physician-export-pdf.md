...
REPORT_FROM_EMAIL=reports@oravi.com   # Resend verified sender address
```
...
function PageFooter({ name }: { name: string }) {
  return (
    <View style={s.footer} fixed>
      <Text style={s.ftTxt}>Oravi · oravi.com · {name}</Text>
    ...
    <Page size="LETTER" style={s.page}>
      ...
      <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 28 }}>
        <View>
          ...
          <Text style={{ fontSize: 8, color: INK_40, marginTop: 4 }}>oravi.com</Text>
        </View>
...
export function buildReportDocument(data: ReportData, logoBase64: string | null) {
  return (
    <Document title={`Oravi Report \u2014 ${data.fullName}`} author="Oravi" creator="oravi.com">
...
export async function POST(req: NextRequest) {
  ...
  if (body.sendEmail && body.recipientEmail) {
    ...
    const { error } = await resend.emails.send({
      from: process.env.REPORT_FROM_EMAIL ?? "reports@oravi.com",
