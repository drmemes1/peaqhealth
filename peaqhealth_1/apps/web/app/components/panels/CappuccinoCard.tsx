...
export interface CappuccinoCardProps {
  status: Status
...
export function CappuccinoCard({ status, icon, title, question, value, valueIsText, unit, pill, scaleItems, sources, explain, pullquote, compact }: CappuccinoCardProps) {
  ...
  return (
    <div style={{
      ...
      {/* TIER 1 — Header */}
      <div className="cappuccino-header" style={{
        background: "#FAF8F2", padding: compact ? "16px 20px 14px" : "24px 28px 20px",
        display: "grid", gridTemplateColumns: `${iconSize}px 1fr auto`, gap: `0 ${compact ? 12 : 18}px`, alignItems: "start",
      }}>
        <div style={{
          width: iconSize, height: iconSize, borderRadius: compact ? 10 : 12,
        ...
        <div style={{ minWidth: 0 }}>
          <h3 style={{ fontFamily: serif, fontSize: titleSize, fontWeight: 500, letterSpacing: "-0.01em", color: "#2C2A24", margin: "0 0 4px", lineHeight: 1.2 }}>
            {title}
          </h3>
          <p style={{ fontFamily: sans, fontSize: compact ? 12 : 13.5, color: "#7A7870", lineHeight: 1.5, margin: 0, maxWidth: 480 }}>
            {question}
          </p>
        </div>
        <div className="cappuccino-value" style={{ minWidth: compact ? 80 : 140, textAlign: "right" }}>
          {valueIsText ? (
            <span style={{ fontFamily: serif, fontSize: compact ? 16 : 20, fontStyle: "italic", color: status === "pending" ? "#9A9894" : "#2C2A24", lineHeight: 1.3 }}>
              {value}
            </span>
          ) : (
            <span style={{ fontFamily: serif, fontSize: compact ? 28 : 38, fontWeight: 500, color: status === "pending" ? "#9A9894" : "#2C2A24", lineHeight: 1, letterSpacing: "-0.02em" }}>
              {value}{unit && <span style={{ fontSize: compact ? 12 : 16, fontWeight: 400, color: "#8C897F", marginLeft: 2 }}>{unit}</span>}
            </span>
          )}
          <div style={{ marginTop: compact ? 4 : 6 }}>
            <span style={{
              fontFamily: sans, fontSize: compact ? 9 : 10, fontWeight: 500,
      ...
      {/* TIER 2 — Measure row */}
      {scaleItems && scaleItems.length > 0 && (
        <div className="cappuccino-measure" style={{
          background: "#E8DCC4", padding: compact ? "12px 20px" : "16px 28px",
          borderTop: "1px solid rgba(44, 42, 36, 0.06)",
          display: "flex", justifyContent: "space-between", alignItems: "center",
          flexWrap: "wrap", gap: 8,
        }}>
          <div style={{ display: "inline-flex", flexWrap: "wrap", gap: compact ? 10 : 16, alignItems: "center" }}>
            {scaleItems.map((item, i) => {
              const dotColor = item.isTarget ? "#6B5B3E" : item.status ? ACCENT[item.status] : "#9A9894"
              return (
                <span key={i} style={{
                  fontFamily: sans, fontSize: compact ? 10.5 : 11.5, color: "#4A4640",
      ...
      {/* TIER 3 — Explain */}
      <div style={{
        background: "#EDE9DE", padding: compact ? "16px 20px 18px" : "22px 28px 24px",
        ...
        <div style={{ fontFamily: serif, fontSize: compact ? 15 : 16, lineHeight: 1.7, color: "#3D3B35", maxWidth: 700 }}>
          {explain}
        </div>
      </div>
      ...
      {pullquote && (
        <div style={{
          ...
          <div style={{ fontFamily: serif, fontSize: 17, lineHeight: 1.55, color: "#F5F3EE", position: "relative" }}>
            {pullquote.body}
          </div>
        </div>
      )}
