...
export async function POST(request: NextRequest) {
  ...
  // Delete sleep_data rows for this source (best-effort, non-fatal)
  const { error: wsdErr, count: wsdCount } = await serviceClient
    .from("sleep_data")
    .delete({ count: "exact" })
    .eq("user_id", user.id)
    .eq("source", provider)
  console.log(`[junction-disconnect] sleep_data deleted for source=${provider}:`, { rows: wsdCount, error: wsdErr?.message ?? null })
