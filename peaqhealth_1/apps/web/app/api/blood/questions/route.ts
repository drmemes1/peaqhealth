...
export async function GET() {
  ...
  const { data: lab } = await supabase.from("lab_results").select("ldl_mgdl, hdl_mgdl, triglycerides_mgdl, total_cholesterol_mgdl, hba1c_pct, glucose_mgdl, hs_crp_mgl, egfr_mlmin, alt_ul, ast_ul, tsh_uiuml, vitamin_d_ngml").eq("user_id", user.id).eq("parser_status", "complete").order("collection_date", { ascending: false }).limit(1).maybeSingle()
