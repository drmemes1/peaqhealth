...
const PROMPT_VERSION = "blood-summary-v1"
...
export async function GET() {
  ...
  const { data: lab } = await supabase.from("lab_results").select(LAB_COLS).eq("user_id", user.id).eq("parser_status", "complete").order("collection_date", { ascending: false }).limit(1).maybeSingle()
  ...
  try {
    ...
    const completion = await openai.chat.completions.create({ model: process.env.OPENAI_MODEL ?? "gpt-4.1-mini", temperature: 0.1, max_tokens: 800, store: false, messages: [{ role: "system", content: NARRATIVE_SYSTEM_BASE + SUMMARY_INSTRUCTION }, { role: "user", content: `Blood panel data:\n${JSON.stringify(lab, null, 2)}` }] })
