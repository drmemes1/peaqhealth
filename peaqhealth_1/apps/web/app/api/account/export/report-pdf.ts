...
function drawPageFooter(doc: InstanceType<typeof PDFDocument>, name: string) {
  ...
  doc.font("Helvetica").fontSize(8).fillColor(INK_40)
  doc.text(`Cnvrg Health \u00b7 peaqhealth.vercel.app \u00b7 ${name}`, MARGIN, footerY, { lineBreak: false })
...
function drawCoverPage(doc: InstanceType<typeof PDFDocument>, data: ReportData, logoBase64: string | null) {
  ...
  if (logoBase64) {
    try {
      ...
      doc.font("Helvetica-Bold").fontSize(16).fillColor(INK).text("Cnvrg Health", MARGIN, MARGIN)
    }
  } else {
    doc.font("Helvetica-Bold").fontSize(16).fillColor(INK).text("Cnvrg Health", MARGIN, MARGIN)
  }
  doc.font("Helvetica").fontSize(8).fillColor(INK_40).text("peaqhealth.vercel.app", MARGIN, MARGIN + 32)
  ...
  // Peaq Age label
  doc.font("Helvetica").fontSize(8).fillColor(INK_40)
  doc.text("CNVRG AGE", MARGIN + pad, curY, { characterSpacing: 1.5 })
  curY += 14
  // Big score number
  ...
  const methCols = [
    ...
    { label: "Oral \u00b7 30 pts", color: GREEN, body: "DNA microbiome sequencing (Zymo Research)." },
  ]
...
function drawSleepPage(doc: InstanceType<typeof PDFDocument>, data: ReportData) {
  ...
  if (noSleep) {
    doc.font("Helvetica").fontSize(10).fillColor(INK_40)
    doc.text("No wearable data on file. Connect a wearable device in Cnvrg Health.", MARGIN, doc.y, { oblique: true })
    drawPageFooter(doc, data.fullName)
    return
  }
...
function drawOralPage(doc: InstanceType<typeof PDFDocument>, data: ReportData) {
  drawPageHeader(doc, `Oral Microbiome \u00b7 DNA Sequencing${data.reportDate ? ` \u00b7 ${fmtDate(data.reportDate)}` : ""}`)

  if (data.shannonDiversity == null) {
    doc.font("Helvetica").fontSize(10).fillColor(INK_40)
    doc.text("No oral microbiome results on file. Order a Cnvrg oral kit to include microbiome data.", MARGIN, doc.y, { oblique: true })
    drawPageFooter(doc, data.fullName)
    return
  }
...
function drawLifestylePage(doc: InstanceType<typeof PDFDocument>, data: ReportData) {
  ...
  doc.font("Helvetica").fontSize(9).fillColor(INK_40)
  doc.text(
    "This report is generated by Cnvrg Health for informational purposes only and is intended to support conversations with your healthcare provider. It is not a medical diagnosis, treatment recommendation, or clinical document. All findings should be reviewed in the context of the patient\u2019s complete medical history by a qualified physician or dentist.\n\nScoring methodology, scientific citations, and full platform documentation available at peaqhealth.vercel.app/science",
    MARGIN, doc.y, { width: CONTENT_W, lineGap: 5 }
  )

  doc.y += 20
  doc.font("Helvetica").fontSize(8).fillColor(INK_40)
  doc.text("Cnvrg Health \u00b7 peaqhealth.vercel.app", MARGIN, doc.y, { lineBreak: false })
...
export async function buildReportDocument(data: ReportData, logoBase64: string | null): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    const doc = new PDFDocument({
      ...
      margin: MARGIN,
      info: {
        Title: `Cnvrg Health Report \u2014 ${data.fullName}`,
        Author: "Cnvrg Health",
        Creator: "peaqhealth.vercel.app",
      },
      bufferPages: true,
    })
