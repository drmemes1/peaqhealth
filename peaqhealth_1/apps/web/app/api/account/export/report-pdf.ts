...
// TODO Phase 5d: Update PDF layout for Oravi Age V5
...
function drawPageFooter(doc: InstanceType<typeof PDFDocument>, name: string) {
  ...
  doc.font("Helvetica").fontSize(8).fillColor(INK_40)
  doc.text(`Oravi \u00b7 peaqhealth.vercel.app \u00b7 ${name}`, MARGIN, footerY, { lineBreak: false })
...
function drawCoverPage(doc: InstanceType<typeof PDFDocument>, data: ReportData, logoBase64: string | null) {
  ...
  if (logoBase64) {
    try {
      ...
      doc.font("Helvetica-Bold").fontSize(16).fillColor(INK).text("Oravi", MARGIN, MARGIN)
    }
  } else {
    doc.font("Helvetica-Bold").fontSize(16).fillColor(INK).text("Oravi", MARGIN, MARGIN)
  }
  ...
  // Oravi Age label
  doc.font("Helvetica").fontSize(8).fillColor(INK_40)
  doc.text("ORAVI AGE", MARGIN + pad, curY, { characterSpacing: 1.5 })
  curY += 14
  // Big score number
...
function drawSleepPage(doc: InstanceType<typeof PDFDocument>, data: ReportData) {
  ...
  if (noSleep) {
    doc.font("Helvetica").fontSize(10).fillColor(INK_40)
    doc.text("No wearable data on file. Connect a wearable device in Oravi.", MARGIN, doc.y, { oblique: true })
    drawPageFooter(doc, data.fullName)
    return
  }
...
function drawOralPage(doc: InstanceType<typeof PDFDocument>, data: ReportData) {
  ...
  if (data.shannonDiversity == null) {
    doc.font("Helvetica").fontSize(10).fillColor(INK_40)
    doc.text("No oral microbiome results on file. Order a Oravi oral kit to include microbiome data.", MARGIN, doc.y, { oblique: true })
    drawPageFooter(doc, data.fullName)
    return
  }
...
function drawLifestylePage(doc: InstanceType<typeof PDFDocument>, data: ReportData) {
  ...
  doc.y += 20
  doc.font("Helvetica").fontSize(8).fillColor(INK_40)
  doc.text("Oravi \u00b7 peaqhealth.vercel.app", MARGIN, doc.y, { lineBreak: false })
...
export async function buildReportDocument(data: ReportData, logoBase64: string | null): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    const doc = new PDFDocument({
      ...
      margin: MARGIN,
      info: {
        Title: `Oravi Report \u2014 ${data.fullName}`,
        Author: "Oravi",
