...
export const METHODOLOGY: MethodologyEntry[] = [
  ...
  },
  {
    ...
    howComputed:
      "HMI = (H2S drivers + CH3SH drivers) × protective modifier × LHM (Lifestyle Halitosis Modifier). H2S pathway weights F. nucleatum (1.0×), S. moorei (1.5×), Veillonella (continuous interaction term per Washio 2014: base weight 0.10× scaled by lactate enhancement 1+ s_mutans/0.5 capped at 2.0× — replaces the prior boolean lactate boost; product capped at absolute contribution 1.0), Leptotrichia wadei (0.5×), Atopobium parvulum (0.5× H2S + 0.6× CH3SH dual-pathway), Selenomonas (magnitude-aware: 0.2× / 0.4× / 0.5× depending on genus total), Eubacterium sulci (0.4×, often null due to V3-V4 limit), Dialister invisus (0.3×). CH3SH pathway weights P. gingivalis (1.0×), Prevotella intermedia / nigrescens (0.8× each), Prevotella denticola (0.6×), Prevotella melaninogenica (v2.5: 0.10× — lowered from 0.2× per OE consultation; universal commensal with no in vitro VSC-driver evidence), Treponema genus (0.7×), T. forsythia (0.5×), conditional S. moorei × Pg/Td synergy (0.5× when both elevated), Eikenella corrodens (0.3×). Protective modifier (parity with perio CDM): protective_score = S. salivarius × 1.0 + Rothia × 0.5 + Haemophilus × 0.3, capped at 15. Modifier ranges 0.40 (full protection) to 1.25 (collapsed). LHM compounds lifestyle factors (mouth breathing, snoring, dry mouth on waking, smoking, GERD frequency, last dental cleaning, tongue scraping, age) capped at 1.60×. v2.5 calibration: 3 categories (low/moderate/high). Pathway attribution (tongue_dominant / gum_dominant / mixed / minimal_pressure) is the primary diagnostic content — pathway = minimal_pressure when category=low (intervention not actionable); otherwise tongue_dominant when H2S > CH3SH × 1.3, gum_dominant when reverse, mixed when within 1.3× of each other. Subjective halitosis routing fires when category=low but LHM > 1.30 — narrative explicitly routes to non-bacterial cause investigation rather than producing a falsely reassuring read.",
    inputs: [
    ...
    thresholds:
      "v2.5 — 3 categories: low < 2.0, moderate 2.0–4.5, high ≥ 4.5. Pathway dominance ratio 1.3× (lowered from 1.5× since pathway now drives the primary diagnostic label rather than a tie-breaker). Subjective halitosis routing trigger: category=low + LHM > 1.30. No published validated thresholds exist for any halitosis algorithm — these are heuristic anchors pending N=200 internal cohort validation.",
    limitations:
