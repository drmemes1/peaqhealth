...
export async function generateWeeklySnapshot(userId: string): Promise<Record<string, unknown> | null> {
  ...
  const oralLines = oralData ? [
    ...
    snap?.shannonDiversity   != null ? `Shannon diversity: ${snap.shannonDiversity}` : null,
    snap?.nitrateReducerPct  != null ? `Nitrate reducers: ${Number(snap.nitrateReducerPct).toFixed(1)}%` : null,
    snap?.periodontalBurden  != null
      ? `Periodontal burden: ${Number(snap.periodontalBurden) > 0.5 ? "elevated" : "within target"}`
      : null,
...
SLEEP THIS WEEK (${thisWeekSleep.length} nights tracked):
Average HRV: ${avgHrv != null ? `${avgHrv.toFixed(1)} ms${avgHrv >= 50 ? " (above target)" : " (below 50ms target)"}` : "no data"}
HRV vs last week: ${hrvTrendPct != null ? `${hrvTrendPct > 0 ? "+" : ""}${hrvTrendPct.toFixed(1)}%` : "no comparison available"}
...
Average REM: ${avgRemPct != null ? `${avgRemPct.toFixed(1)}%${avgRemPct >= 18 ? " (above target)" : ""}` : "no data"}

CURRENT SCORES:
Total: ${currentSnapshot?.score ?? "no data"}/100
Sleep: ${currentSnapshot?.sleep_sub ?? "no data"}/27
Blood: ${currentSnapshot?.blood_sub ?? "no data"}/33
Oral: ${currentSnapshot?.oral_sub ?? "no data"}/27
