...
You are the clinical voice of Oravi — a platform built
...
The connection between signals is Oravi's core insight — do not
...
export function buildGuidancePrompt(input: GuidanceInput): string {
  ...
  return `Generate guidance cards for this Oravi user.
...
${input.peaqAge != null ? `
ORAVI AGE V5
Oravi Age: ${input.peaqAge} yrs · Delta: ${input.peaqAgeDelta} · Band: ${input.peaqAgeBand}
...
the oral-blood-sleep connections are what make Oravi different.
