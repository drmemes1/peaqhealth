...
export async function recalculateScore(
  ...
  const [wearableRes, labsRes, oralRes, lifestyleRes, manualSleepRes, whoopRes] = await Promise.all([
    ...
    supabase.from("oral_kit_orders").select("*").eq("user_id", userId).in("status", ["results_ready", "scored"]).not("shannon_diversity", "is", null).order("ordered_at", { ascending: false }).limit(1).maybeSingle(),
