...
7. Return values in the unit the lab printed (in unitFound). Do NOT skip a marker because its unit differs from the registry's expected unit — Function Health, Quest, and many premium labs report Lp(a) in nmol/L, glucose in mmol/L, vitamin D in nmol/L, B12 in pmol/L, testosterone in nmol/L, etc. Always extract; the post-processing layer handles conversion. EXAMPLE: if the lab prints "Lipoprotein (a) 75 nmol/L", return value=75 with unitFound="nmol/L" for the lipoprotein_a_mgdl marker, even though the expectedUnit is mg/dL.
